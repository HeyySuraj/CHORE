...
export class MongoArchiveService {
    ...
    async archiveOldData() {
        try {
            ...
            await this.connect();
            // const dbName = '1spoc-staging';
            const collectionArray = ["Workflows", "users"];
    ...
    async createMonthlyCollection(collectionInstance, baseCollectionName) {
        ...
        try {
            ...
            console.log(`${LOG_PREFIX} Creating Monthly Collection START: ${new Date().toLocaleString()} `);


            const dateUpperLimit = new Date();
            dateUpperLimit.setMonth(dateUpperLimit.getMonth() - SKIP_LATEST_MONTHS - 1);
            // set to start of that month
            dateUpperLimit.setDate(1);
            ...
            while (true) {
                ...
                const currentOldestDocument = await collectionInstance.findOne({ createdAt: { $exists: true } }, {}, { sort: { createdAt: 1 } });

                // BREAK 1 : This condition occure when we want to empty whole collection
                if (!currentOldestDocument) {
                    console.log(`${LOG_PREFIX} : No document found for CollectionName: ${baseCollectionName}, iterationCount : ${_iterationCount}`);
                    break;
                }

                let targetDate = currentOldestDocument.createdAt;
                console.log("targetDate", targetDate);


                // BREAK 2 : TARGET RANGE EXCEEDED : Keep Last 3 months data 
                if (targetDate > dateUpperLimit) { // If need whole collection monthwise dump just comment this check
                    console.log(`${LOG_PREFIX} Archived all monthly data of collection : ${baseCollectionName} | Creating Monthly Collection END: ${new Date().toLocaleString()}`);
                    break;
                }

                const year = targetDate.getFullYear()
                const month = targetDate.getMonth() + 1; // get month gives currnetMonth - 1

                const { start, end } = getMonthRange(year, month);
                console.log(`${LOG_PREFIX} : ${start} < Processing range < ${end}`);
                ...
                if (_prevCollectionName !== _dateWiseCollectionName) {
                    if (_prevCollectionName) { // we got new collection hence dump the previous one 
                        ...
                        try {
                            ...
                            console.log(`${LOG_PREFIX} : DUMPING START FOR `, _prevCollectionName);
                            await this.dumpCollection(_prevCollectionName, DUMP_DIR_NAME);

                            console.log(`${LOG_PREFIX} : ZIPPING DUM FILES START FOR `, _prevCollectionName);
                            await zipFolder(
                                path.join(__dirname, `${DUMP_DIR_NAME}/${_prevCollectionName}`),
                                path.join(`${__dirname}/${ZIP_DIR_NAME}`, `${_prevCollectionName}.zip`),
                            )

                        } catch (error) {
                            console.log(`Error occured while taking dump & zip file for ${_prevCollectionName}`, error.message);
                ...
                if (currentDocumentsLength === 0) {
                    ...
                    console.log(`${LOG_PREFIX} [BATCH PROCCESSING END] | DUMPING COLLECTION : ${baseCollectionName} START`);

                    await this.dumpCollection(_dateWiseCollectionName, DUMP_DIR_NAME);

                    console.log(`${LOG_PREFIX} DUMPING COLLECTION : ${baseCollectionName} START`);
                }
                ...
                console.log("Processed Documents Count : ", _processDocumentsCount);
            }
