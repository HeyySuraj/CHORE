...
var (
        ...
        privPath = flag.String("priv", "/home/ubuntu/private_key.pem", "private_key.pem")
...
func handler(priv *rsa.PrivateKey) http.HandlerFunc {
        return func(w http.ResponseWriter, r *http.Request) {
                ...
                if err != nil {
                        log.Printf("[Error] decryptFlow failed: %v %s", err, plaintext)
                        http.Error(w, "decrypt failed: "+err.Error(), http.StatusInternalServerError)
                        return
                }

                // Pretty-print JSON object to logs
                // pretty, _ := json.MarshalIndent(parsed, "", "  ")
                // log.Printf("[Debug] Decrypted JSON:\n%s", string(pretty))

                // Extract flow_token to decide backend
                var first string
                if ft, ok := parsed["flow_token"].(string); ok && ft != "" {
                        parts := strings.SplitN(ft, "|", 2)
                        if len(parts) > 0 {
                                first = strings.TrimSpace(parts[0])
                                // log.Printf("[Info] Waba Number %s", first)
                        }
                }

                // Special handling: if action == "ping", force default route
                if act, ok := parsed["action"].(string); ok && act == "ping" {
                        // log.Printf("[Info] Action=ping detected, forcing default route")
                        first = "DEFAULT"
                }
                ...
                var baseURL string
                switch first {
                case "919461062705":  // RISL ON PREM
                        // baseURL = "http://127.0.0.1:8002/process/pf2/execute" // IP ADDRESS
                        baseURL = "https://dummy.in/WABOT/process/pf2/execute" // DOMAIN URL
                
                case "919922970108": // RISL Testing
                        baseURL = "https://dummy.ai/process/pf2/execute"
                default:
                        baseURL = "https://dummy.ai/process/pf2/execute"
                }

                log.Printf("BaseURL: %s, wabaNumber: %s", baseURL, first)
...
func main() {
        ...
        if _, err := os.Stat(*privPath); os.IsNotExist(err) {
                log.Fatalf("private key file does not exist: %s %s", *privPath, wd)
        }
